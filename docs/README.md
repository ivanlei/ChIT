...
- sniffs: Every monster/phylum you currently have "sniffed" (Transcendent Olfaction, Gallapagosian Mating Call, McHugeLarge Slash, Long Con, the Nosy Nose / Red-Nosed Snapper familiars, etc.) with the source icon and, on the right, casts or turns remaining for that tracker (∞ if it has no daily limit). Off by default; add `sniffs` to a layout to use it.
